    ...
    return itsButton ? (
      <button
       ...
       bg-slate-200 hover:bg-slate-600 hover:text-white focus:bg-slate-600 focus:text-white
         flex items-center gap-2 border py-1.5 px-2 rounded-md border-slate-400 cursor-pointer truncate`}
        onClick={() => navigate(whereToNav)}
      >
        {icon}
        <div
          className={`${theresClass ? 'w-12 hidden truncate md:w-full xs:visible xs:flex text-xl lg:w-16 xl:w-full' : textClass}`}
        >
          {textIcon}
        </div>
      </button>
